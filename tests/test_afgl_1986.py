...
def test_parse_returns_dataframe(name) -> None:
...
@pytest.mark.parametrize("name", [n for n in afgl_1986.Name])
def test_parse_identifier(name) -> None:
    """Handles all supported identifier values."""
    afgl_1986.parse(name=name)


@pytest.mark.parametrize("name", [n for n in afgl_1986.Name])
def test_to_xarray_returns_dataset(name) -> None:
    """Returns a Dataset."""
    df = afgl_1986.parse(name=name)
...
@pytest.mark.parametrize("name", [n for n in afgl_1986.Name])
def test_to_xarray_all_coords(name) -> None:
...
@pytest.mark.parametrize("name", [n for n in afgl_1986.Name])
def test_to_xarray_all_data_vars(name) -> None:
...
@pytest.mark.parametrize("name", [n for n in afgl_1986.Name])
def test_to_xarray_attrs(name) -> None:
